...
def validate_weather_icon(icon: str) -> Optional[str]:
    ...
    if icon and isinstance(icon, str) and icon.startswith('/dl/'):
        return icon
    return None
...
def get_value_by_icon(page, alt_text: str) -> Optional[str]:
    try:
        block = page.locator(f"img[alt*='{alt_text}']").first.locator("xpath=..")
        return block.locator("p.font-medium").first.inner_text()
    except Exception:
        return None
...
def crawl_city_data(page, city: Dict) -> Optional[Dict]:
    ...
    try:
        ...
        page.goto(city['url'], wait_until="domcontentloaded", timeout=60000)
        
        # Wait for main AQI value
        page.wait_for_selector("p.text-lg.font-medium")
        
        # Extract raw values
        aqi_raw = page.query_selector("p.text-lg.font-medium").text_content()
        weather_icon_raw = page.locator("img[alt*='thời tiết']").first.get_attribute("src")
        
        # Wind
        wind_block = page.locator("img[alt*='Gió']").first.locator("xpath=..")
        speed_value = wind_block.locator("p.font-medium").first.inner_text()
        speed_unit  = wind_block.locator("p").nth(1).inner_text()
        wind_speed_raw = f"{speed_value.strip()} {speed_unit.strip()}"
        
        # Humidity (dùng helper)
        humidity_raw = get_value_by_icon(page, "Độ ẩm")
        ...
        try:
            with sync_playwright() as p:
                try:
                    ...
                    page.set_default_timeout(60000)  # 15 seconds timeout
